...
function selectStratified(pool, n) {
  ...
  if (picked.length < n) {
    ...
    for (const s of remaining) {
      ...
      if (!pickedIds.has(_stimKey(s))) {
        picked.push(s);
        pickedIds.add(_stimKey(s));
      }
    }
  }
...
function _stimKey(s) {
  return s.stim_id ?? s.audio_file ?? s.audio_url ?? JSON.stringify(s);
}
  ...
  state.player = new AudioPlayer(url, {
    ...
    onProgress: (ct, dur) => {
      ...
      if (!state.listenGateOpen && state.listenedSeconds >= LISTEN_MIN_S) {
        ...
        const hint2 = document.getElementById('listen-hint');
...
function preloadOne(s) {
  ...
  if (state.preloadPlayer) {
    state.preloadPlayer.destroy();
    state.preloadPlayer = null;
  }
  state.preloadPlayer = new AudioPlayer(s.audio_url);
}
